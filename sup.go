...
func (sup *Stackup) Run(network *Network, envVars EnvList, commands ...*Command) error {
	...
	if network.Bastion != "" {
		...
		if bastion.privateKeyFile == "" {
			bastion.privateKeyFile = network.IdentityFile
		}
	...
	for i, host := range network.Hosts {
		...
		go func(i int, host string) {
			...
			if remote.privateKeyFile == "" {
				remote.privateKeyFile = network.IdentityFile
			}
	...
	for _, cmd := range commands {
		...
		for _, task := range tasks {
			...
			if task.Input != nil {
				go func() {
					if closer, ok := task.Input.(io.Closer); ok {
						defer closer.Close()
					}
